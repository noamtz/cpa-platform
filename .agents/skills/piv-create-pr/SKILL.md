...
1. If `the user's request` contains `--base <branch>`, use that.
...
| Existing PR for this branch (`python tooling/github.py pr list --head $(git branch --show-current) --json url`) | STOP and print the URL. |
| Clean, commits ahead, no PR | PROCEED |

## Phase 2 — Gather context for the body

- **Project conventions:** if `.agents/references/conventions.md` exists, read its `## pr` section — its rules
...
```bash
python tooling/github.py pr create --base "{base}" --title "{type}: {concise description}" --body "$(cat <<'EOF'
## Summary
...
```bash
python tooling/github.py pr view --json number,url,title,baseRefName,headRefName
```
