...
# Investigate Issue #<issue-number> (Root-Cause Analysis)

## Objective

Investigate GitHub issue #<issue-number> from this repository, identify the root cause, and document findings for future implementation.
...
- GitHub CLI installed with the `noamtz` credential available through `python tooling/github.py`
...
```markdown
# Root Cause Analysis: GitHub Issue #<issue-number>
...
2. Run `$piv-implement-issue` with issue #<issue-number> to implement the fix
