...
## Feature: the user's request

## Resolve the input first

`the user's request` is either a **tracker ticket** (a key like `ACC-30`, or a Jira / Linear / GitHub issue URL) or a
...
- Find relevant documentation in docs/, ai_docs/, .agents/references or ai-wiki if available
