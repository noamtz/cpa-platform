...
# Implement Issue Fix: GitHub Issue #<issue-number>

Resolve `<issue-number>` from the user's request before running any command and use that resolved value consistently.
...
```bash
python tooling/github.py issue view <issue-number>
```
...
- Review the GitHub issue details (issue #<issue-number>)
...
- **Drift check:** read each file the RCA names and compare against the RCA's "current code" snippets / line refs.
  ...
  `$piv-investigate-issue` for issue #<issue-number> rather than implementing a stale plan.
...
- **On the base branch, clean tree?** Create a fix branch — `git checkout -b fix/issue-<issue-number>-<slug>` (detect
  the base with `git symbolic-ref refs/remotes/origin/HEAD`; never hardcode `main`).
- **Already on a feature/fix branch?** Use it (warn if its name doesn't reference #<issue-number>).
...
**Test implementation:**
```python
def test_issue_123_fix():
    """Regression test for the resolved issue number."""
...
**GitHub Issue #<issue-number>**: [Brief title]
...
```
fix(scope): resolve GitHub issue #<issue-number> - [brief description]
...
**Note:** Using `Fixes #<issue-number>` in the commit message will automatically close the GitHub issue when merged to the default branch.

### Optional: Update GitHub Issue

**Add implementation comment to issue:**
```bash
python tooling/github.py issue comment <issue-number> --body "Fix implemented in commit [commit-hash]. Ready for review."
```

**Update issue labels (if needed):**
```bash
python tooling/github.py issue edit <issue-number> --add-label "fixed" --remove-label "bug"
```

**Close the issue (if not using auto-close via commit message):**
```bash
python tooling/github.py issue close <issue-number> --comment "Fixed and merged."
```

## Notes

- If the RCA document is missing or incomplete, request it be created first with `$piv-investigate-issue` for issue #<issue-number>
...
- The commit message `Fixes #<issue-number>` will link the commit to the GitHub issue
