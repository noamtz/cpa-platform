...
- **Project documents:** Store canonical PRDs and architecture documents as Markdown pages in the GitHub Wiki configured in `.github/project-documents.json`. Represent each with a repository issue attached to the GitHub Project: a PRD uses one master epic issue, while architecture uses a linked child/tracker issue. Store implementation plans as canonical Markdown under `.agents/plans/`; RCAs and delivery reports remain canonical in their tracker issue bodies. Do not duplicate Wiki documents or implementation plans in repository issues, and do not create a fallback when the Wiki is unavailable; report the missing Wiki, Project identity, or authentication prerequisite instead. Keep code-operational contracts in this repository. Read `.agents/references/github-project-documents.md` before creating, reading, or updating a project artifact.
